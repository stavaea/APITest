...
class TestMain extends PHPUnit_Framework_TestCase
{
   ...
   //传参正确，返回二级分类列表正确，接口写死，返回1000、2000、3000、0
    ...
    public function testRecommendsList()
    {
        ...
        $this->assertEquals('1', $result['result']['recommends']['2']['list']['0']['courseId'],'url:'.$this->url.'   Post data:'.json_encode($this->postData));
        $this->assertEquals('test环境直播课', $result['result']['recommends']['2']['list']['0']['title'],'url:'.$this->url.'   Post data:'.json_encode($this->postData));
        $this->assertEquals('1', $result['result']['recommends']['2']['list']['0']['courseType'],'url:'.$this->url.'   Post data:'.json_encode($this->postData));
        $this->assertEquals('高能测试', $result['result']['recommends']['2']['list']['0']['orgSubname'],'url:'.$this->url.'   Post data:'.json_encode($this->postData));
    }
   //推荐模块验证
    public function testRecommendsCheckTotal()
    {
        ...
        $this->assertEquals('0',$result['result']['recommends']['0']['list']['0']['total'],'url:'.$this->url.'   Post data:'.json_encode($this->postData));
    }
