...
class TestUserInfoUpdate extends PHPUnit_Framework_TestCase
{
    ...
    public function testUserInfoUpdateParamsError($uid = "")
    {
        ...
        $this->assertEquals('200', $this->http->HttpApiGetCode($httpurl));
        $result = json_decode($this->http->HttpApiPost($httpurl, json_encode("")), true);
    ...
    public function testUserInfoUpdateGenderSuccess($uid = "22415")
    {
        ...
        $result = json_decode($this->http->HttpApiPost($httpurl, json_encode($postData)), true);
