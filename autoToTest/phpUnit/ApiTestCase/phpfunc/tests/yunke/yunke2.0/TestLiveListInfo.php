...
class TestLiveListInfo extends PHPUnit_Framework_TestCase
{
    ...
    public function testLiveCourseInfo()
    {
        ...
        $this->assertEquals('52', $result['result']['data'][0]['courseId'],'url:'.$this->url.'   Post data:'.json_encode($this->postData));
        $this->assertEquals('899', $result['result']['data'][0]['planId'],'url:'.$this->url.'   Post data:'.json_encode($this->postData));
        $this->assertEquals('1班', $result['result']['data'][0]['className'],'url:'.$this->url.'   Post data:'.json_encode($this->postData));
        $this->assertEquals('52', $result['result']['data'][0]['classId'],'url:'.$this->url.'   Post data:'.json_encode($this->postData));
        $this->assertEquals('0', $result['result']['data'][0]['trys'],'url:'.$this->url.'   Post data:'.json_encode($this->postData));
        $this->assertEquals('课程虚拟价-免费', $result['result']['data'][0]['courseName'],'url:'.$this->url.'   Post data:'.json_encode($this->postData));
        $this->assertEquals('第2课时', $result['result']['data'][0]['sectionName'],'url:'.$this->url.'   Post data:'.json_encode($this->postData));
        $this->assertEquals('1', $result['result']['data'][0]['status'],'url:'.$this->url.'   Post data:'.json_encode($this->postData));
        $this->assertEquals('15:00', $result['result']['data'][0]['stime'],'url:'.$this->url.'   Post data:'.json_encode($this->postData));
    }
