...
class TestTeacherComment extends PHPUnit_Framework_TestCase
{
    ...
    public function testHaveReply()
    {
        ...
        $this->assertArrayHasKey('manage_name', $result['result']['list'][1]);//回复人名称
        $this->assertArrayHasKey('contents', $result['result']['list'][1]);//回复内容
        $this->assertArrayHasKey('replay_time', $result['result']['list'][1]);//回复时间
        $this->assertArrayHasKey('status', $result['result']['list'][1]);//暂时没用
    }
}